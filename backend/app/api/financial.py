...
def list_composition_details(
    date_from: str = Query(pattern=r"^\d{4}-\d{2}-\d{2}$"),
    date_to: str = Query(pattern=r"^\d{4}-\d{2}-\d{2}$"),
    kind: str = Query(pattern=r"^(expense|commission|benefit|refund)$"),
):
    if date_from > date_to:
        raise HTTPException(status_code=400, detail="开始日期不能晚于结束日期")
...
def create_commission(data: CommissionCreate, request: Request):
    return financial_record_service.create_commission(data, _operator(request)).model_dump(mode="json")


@router.put("/commissions/{record_id}")
def update_commission(record_id: str, data: CommissionCreate, request: Request):
    try:
        return financial_record_service.update_commission(record_id, data, _operator(request)).model_dump(mode="json")
    except ValueError as error:
...
def create_staff_benefit(data: StaffBenefitCreate, request: Request):
    return financial_record_service.create_benefit(data, _operator(request)).model_dump(mode="json")


@router.put("/staff-benefits/{record_id}")
def update_staff_benefit(record_id: str, data: StaffBenefitCreate, request: Request):
    try:
        return financial_record_service.update_benefit(record_id, data, _operator(request)).model_dump(mode="json")
    except ValueError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
