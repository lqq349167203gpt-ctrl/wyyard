...
from app.services import oh_card_reading_service, oh_card_reading_session_service
...
def list_readings(page: int | None = Query(None, ge=1), page_size: int | None = Query(None, ge=1, le=100), customer_ids: str | None = Query(None), nickname: str | None = Query(None), closer_name: str | None = Query(None)):
    ...
    for item in items_dict:
        ...
        if cid not in remaining_cache:
            remaining_cache[cid] = oh_card_reading_session_service.get_remaining_count(cid)
...
def get_reading(reading_id: str):
    ...
    result = reading.model_dump() if hasattr(reading, "model_dump") else reading
    result["effective_remaining"] = oh_card_reading_session_service.get_remaining_count(reading.customer_id)
    return result
