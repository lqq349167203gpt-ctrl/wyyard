...
def _load():
    ...
    if isinstance(raw, list) and raw and isinstance(raw[0], str):
        ...
        _types = []
    # [数据迁移] 发布控制已下沉到每条活动记录，移除旧类型级字段。
    removed_legacy_field = False
    for item in _types:
        if "show_in_client" in item:
            item.pop("show_in_client", None)
            removed_legacy_field = True
    # 合并课程数据中已有的类型（向后兼容）
...
def create_course_type(name: str, organization_id: str = "", list_image: str = "",
...
def rename_course_type(old_name: str, new_name: str) -> bool:
    ...
    if old_name not in names:
        return False
    current = _types[names.index(old_name)]
    if current.get("category") == "other":
        raise ValueError("系统内置活动名称不允许修改")
...
def update_course_type(name: str, organization_id: Optional[str] = None,
    ...
    is_system_activity = _types[idx].get("category") == "other"
    if organization_id is not None and not is_system_activity:
    ...
    if category is not None and not is_system_activity:
...
def delete_course_type(name: str) -> bool:
    ...
    if name not in names:
        return False
    if _types[names.index(name)].get("category") == "other":
        raise ValueError("系统内置活动不允许删除")
