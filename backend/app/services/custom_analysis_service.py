...
METRIC_LABELS = {
    ...
    "invited_customers": ("邀约总人数", "人", "number"),
    "invitation_visits": ("邀约总人次", "人次", "number"),
    "cancelled_customers": ("取消人数", "人", "number"),
    "cancelled_visits": ("取消人次", "人次", "number"),
...
def _metric_values(rows: list[dict[str, Any]]) -> dict[str, int | float]:
    return {
        ...
        "invitation_visits": sum(int(row.get("invitation_count_period") or 0) for row in rows),
        "cancelled_customers": sum(1 for row in rows if (row.get("cancelled_count_period") or 0) > 0),
        "cancelled_visits": sum(int(row.get("cancelled_count_period") or 0) for row in rows),
...
def _local_plan(query: str, actor_id: str) -> AnalysisPlan:
    ...
    metric_keywords = [
        ...
        ("invited_customers", ["邀约总人数", "邀约成功人数", "邀约人数"]),
        ("invitation_visits", ["邀约总人次", "邀约成功人次", "邀约人次"]),
        ("cancelled_customers", ["取消人数", "取消邀约人数"]),
        ("cancelled_visits", ["取消人次", "取消邀约人次"]),
