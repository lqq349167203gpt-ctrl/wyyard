...
def test_product_statistics_filters_healing_teacher_and_keeps_teacher_options(monkeypatch):
    ...
    assert "60次卡" in result["card_type_names"]
    assert result["card_type_amounts"]["60次卡"] == 100
    assert result["card_type_counts"]["60次卡"] == 1
    assert result["card_type_persons"]["60次卡"] == 1
