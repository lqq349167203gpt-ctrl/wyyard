                          ...
                          <span className={voided ? "text-[#c4506a] line-through" : expired ? "text-[#c4506a]" : notStarted ? "text-[#8f959e]" : ""}>
                            ...
                            {voided
                              ...
                              : s.remaining === "不限" || s.total_purchased === "不限"
                                ? ` 不限次（${s.name}扣卡${(s.unlimited_deductions || 0) + (s.activity_deductions || 0)}次${(s.manual_deductions || 0) > 0 ? `/销卡${s.manual_deductions}次` : ""}${(s.internal_course_deductions || 0) > 0 ? `/内部课程抵扣${s.internal_course_deductions}次` : ""}）`
                                : typeof s.remaining === "number" && typeof s.total_purchased === "number"
                                  ? ` ${Math.max(0, s.remaining)}次/${s.total_purchased}次（${s.name}扣卡${(s.activity_deductions || 0) + (s.unlimited_deductions || 0)}次${(s.manual_deductions || 0) > 0 ? `/销卡${s.manual_deductions}次` : ""}${(s.internal_course_deductions || 0) > 0 ? `/内部课程抵扣${s.internal_course_deductions}次` : ""}）`
                                  : ''}
