...
import { accountApi, financialApi } from "@/lib/api"
import type { AccountLight, CommissionRecord } from "@/lib/api"
...
export default function CommissionRecordsPage() {
  ...
  useEffect(() => { accountApi.listLight().then(setAccounts).catch(() => {}) }, [])
  const fetchRecords = useCallback((page: number, pageSize: number) => financialApi.listCommissions(page, pageSize, monthRef.current), [])
  ...
  const save = async () => {
    ...
    if (!form.month || !form.person_name.trim() || !Number.isFinite(amount) || amount <= 0) { setError("请完整填写月份、人员和有效金额"); return }
  ...
  return <div className="dv-root flex h-full flex-col gap-3 bg-[#f4f5f6] p-4">
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl bg-white shadow-[0_2px_4px_rgba(33,38,49,.05)]"><div className="flex flex-wrap items-center gap-2 border-b border-[#f0f0f0] px-4 py-2.5"><input type="month" value={month} onChange={(event) => { setMonth(event.target.value); monthRef.current = event.target.value; pager.goToPage(1) }} className={`h-8 rounded-[7px] border border-[#e1e4e7] bg-white px-2.5 text-[12px] outline-none focus:border-[#3370ff] ${month ? "text-[#2b2f36]" : "text-[#a8b1bd]"}`} /><button className="flex h-8 items-center gap-1 rounded-[4px] border border-[#dee0e3] bg-white px-4 text-[12px] text-[#4e535a] hover:bg-[#f5f6f7]" onClick={() => { setMonth(""); monthRef.current = ""; pager.goToPage(1) }}><X className="h-3.5 w-3.5" />清空</button><div className="flex-1" /><Button size="sm" className="h-8 bg-[#212631] text-[12px] text-white hover:bg-[#303641]" onClick={openCreate}><Plus className="mr-1 h-3.5 w-3.5 text-[#a3c0ff]" />新增分成</Button></div>
      {pager.loading ? <div className="flex flex-col items-center justify-center gap-2 py-16"><Inbox className="h-8 w-8 text-[#d0d3d6]" /><span className="text-[12px] text-[#8f959e]">加载中...</span></div> : pager.totalItems === 0 ? <div className="flex flex-col items-center justify-center gap-2 py-16"><Inbox className="h-8 w-8 text-[#d0d3d6]" /><span className="text-[12px] text-[#8f959e]">暂无数据</span></div> : <Table style={{ tableLayout: "fixed" }}><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="pl-4" style={{ width: "120px" }}>分成月份</TableHead><TableHead style={{ width: "160px" }}>人员</TableHead><TableHead style={{ width: "120px" }}>金额</TableHead><TableHead>备注</TableHead><TableHead style={{ width: "100px" }}>创建人</TableHead><TableHead className="pr-4 text-right" style={{ width: "88px" }}>操作</TableHead></TableRow></TableHeader><TableBody>{pager.paginatedItems.map(item => <TableRow key={item.id} className="group hover:bg-[#f7f8fa]"><TableCell className="pl-4 text-[12px] text-[#2b2f36] tabular-nums">{item.month}</TableCell><TableCell><span className="text-[13px] font-medium text-[#212631]">{item.person_name}</span></TableCell><TableCell className="text-[12px] text-[#c4506a] tabular-nums">¥{item.amount.toLocaleString()}</TableCell><TableCell><span className="block truncate text-[12px] text-[#a8b1bd]" title={item.notes}>{item.notes || <EmptyValue />}</span></TableCell><TableCell className="text-[12px] text-[#a8b1bd]">{item.created_by || <EmptyValue />}</TableCell><TableCell className="pr-4 text-right"><div className="flex justify-end gap-0.5 opacity-0 transition-opacity group-hover:opacity-100"><Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => openEdit(item)}><Pencil className="h-3.5 w-3.5" /></Button><Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={async () => { await financialApi.deleteCommission(item.id); pager.refresh() }}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button></div></TableCell></TableRow>)}</TableBody></Table>}
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="w-[400px] max-w-[90vw] gap-0 p-0" initialFocus={false}><DialogHeader className="border-b border-[#f0f0f0] px-6 pb-2 pt-3"><DialogTitle className="text-[14px] font-normal">{editing ? "编辑分成" : "新增分成"}</DialogTitle></DialogHeader><div className="space-y-4 px-6 py-5">
      ...
      <label className="grid grid-cols-[64px_1fr] items-center gap-3"><span className="text-right text-[12px] text-[#4e535a]">人员</span><div><Input list="commission-accounts" value={form.person_name} placeholder="输入姓名搜索" onChange={e => { const account = accounts.find(a => a.owner === e.target.value || a.username === e.target.value); setForm({ ...form, person_name: e.target.value, person_id: account?.id || "" }) }} /><datalist id="commission-accounts">{accounts.map(a => <option key={a.id} value={a.owner || a.username}>{a.username}</option>)}</datalist></div></label>
