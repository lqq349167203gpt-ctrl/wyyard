...
import {
  ohCardReadingSessionApi, ohCardReadingApi, uploadApi,
  type OhCardReadingSession, type Customer, type GroupCaseCustomerSearchResult,
} from "@/lib/api"

interface DayVisit { id: string; nickname: string; member_type: string }

export interface OcrActions {
  handleOpenEdit: (session: OhCardReadingSession) => void
  handleOpenMaterials: (session: OhCardReadingSession) => void
  handleOpenMembers: (session: OhCardReadingSession) => void
  handleDrop: (session: OhCardReadingSession, customer: { customer_id: string }) => void
  deleteId: string | null
  setDeleteId: (id: string | null) => void
}

interface UseOcrDialogsProps {
...
export function useOcrDialogs({
  ...
  const [materialsRecord, setMaterialsRecord] = useState<OhCardReadingSession | null>(null)
  ...
  const [membersRecord, setMembersRecord] = useState<OhCardReadingSession | null>(null)
  ...
  // ===== Handlers =====
  const handleOpenEdit = (session: OhCardReadingSession) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingRecord) {
        await ohCardReadingSessionApi.update(editingRecord.id, data)
      } else {
        await ohCardReadingSessionApi.create(data)
      }
      setDialogOpen(false)
      onReload()
    } catch (error) {
  ...
  // 资料
  const handleOpenMaterials = (session: OhCardReadingSession) => {
  ...
  // 成员
  const handleOpenMembers = (session: OhCardReadingSession) => {
  ...
  const handleAddParticipant = async (customer: GroupCaseCustomerSearchResult) => {
    ...
    try {
      await ohCardReadingSessionApi.update(membersRecord.id, { participant_ids: newIds } as any)
  ...
  const handleMemberSetHost = async (customer: GroupCaseCustomerSearchResult) => {
    ...
    if (customer.remaining === 0) return
    try {
      await ohCardReadingSessionApi.update(membersRecord.id, { host_id: customer.id, host_name: customer.nickname || customer.name } as any)
  ...
  const handleMemberRemoveHost = async () => {
    if (!membersRecord) return
    try {
      await ohCardReadingSessionApi.update(membersRecord.id, { host_id: "", host_name: "" } as any)
  ...
  const handleDrop = async (session: OhCardReadingSession, customer: { customer_id: string }) => {
    ...
    if (ids.includes(customer.customer_id) || customer.customer_id === session.host_id || customer.customer_id === session.achiever_id) return
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm p-0 gap-0">
          <DialogHeader className="px-6 pt-5 pb-4 border-b">
            <DialogTitle className="text-base">{editingRecord ? "编辑OH卡梳理" : "新增OH卡梳理"}</DialogTitle>
          </DialogHeader>
        ...
        <DialogContent className="max-w-md w-full p-0 gap-0 overflow-hidden">
          ...
          <div className="px-6 py-5 space-y-4 overflow-hidden">
            <div className="flex items-center justify-between gap-3">
              <span className="text-xs text-[#4e535a] truncate">{materialsRecord?.description || "OH卡梳理"}</span>
              <div className="shrink-0">
                <input type="file" id="ocr-materials-upload" className="hidden" onChange={handleUploadMaterial} />
                <Button size="sm" className="h-7 text-xs" disabled={uploading} onClick={() => document.getElementById("ocr-materials-upload")?.click()}>
                  {uploading ? "上传中..." : "上传文件"}
                </Button>
              </div>
            </div>
        ...
        <DialogContent className="max-w-3xl p-0 gap-0">
          ...
          {membersRecord && (
            <div className="flex max-h-[65vh]">
              ...
              >
                <div className="text-[12px] text-[#8f959e]">{membersRecord.description || "OH卡梳理"} · {membersRecord.date}</div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>确定要删除这条OH卡梳理吗？此操作不可撤销。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
