...
import { Plus, Trash2, Edit, Loader2, Zap, X } from "lucide-react"
import {
...
import { energyKnotApi, type EnergyKnot, type CustomerSearchResult } from "@/lib/api"
import { usePagination } from "@/hooks/use-pagination"
import { PaginationBar } from "@/components/pagination-bar"

export function EnergyKnotsContent({ embedded }: { embedded?: boolean } = {}) {
  const [knots, setKnots] = useState<EnergyKnot[]>([])
  ...
  const { paginatedItems, currentPage, totalPages, totalItems, goToPage, startIndex, endIndex } = usePagination(knots)
  ...
  const handleSearch = (keyword: string, target: "user" | "closer") => {
    ...
    searchTimeoutRef.current = window.setTimeout(async () => {
      setSearching(true)
      try {
        const results = await energyKnotApi.searchCustomers(keyword)
  ...
  const handleOpenEdit = (item: EnergyKnot) => {
    setEditingKnot(item)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingKnot) {
        await energyKnotApi.update(editingKnot.id, data)
      } else {
        await energyKnotApi.create(data)
      }
      setDialogOpen(false)
      load()
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return
    try {
      await energyKnotApi.delete(deleteId)
      setDeleteId(null)
      load()
    } catch (e: any) {
  ...
  const totalAmount = knots.reduce((sum, c) => sum + c.amount, 0)
  const totalCount = knots.reduce((sum, c) => sum + c.purchase_count, 0)
    ...
    <>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground mt-[6px]">
          {knots.length > 0 && (
            <span>共 {knots.length} 条记录，{totalCount} 次购买，¥{totalAmount.toLocaleString()}</span>
          )}
        </p>
      ...
      <div className="bg-white rounded-lg">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground">暂无能量结记录</p>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-[70px_1fr] items-start gap-2" ref={searchTarget === "user" ? dropdownRef : undefined}>
              ...
              <div className="relative">
                ...
                />
                {!editingKnot && formNickname && searchTarget !== "user" && (
                  <button
  ...
  return <div className="px-6 pt-12 pb-6 space-y-3"><h1 className="text-lg font-semibold">能量结</h1>{content}</div>
}

export default function EnergyKnotsPage() {
  return <EnergyKnotsContent />
}
