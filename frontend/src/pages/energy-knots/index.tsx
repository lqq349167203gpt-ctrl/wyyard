...
import { Plus, Trash2, Edit, Zap, X } from "lucide-react"
import {
...
import { customerApi, energyKnotApi, type Customer, type EnergyKnot } from "@/lib/api"
...
export function EnergyKnotsContent({ embedded }: { embedded?: boolean } = {}) {
  ...
  const fetchFn = useCallback(async (page: number, pageSize: number) => {
    if (!customersReadyRef.current) {
      return { items: [] as EnergyKnot[], total: 0, page: 1, page_size: pageSize, total_pages: 0 }
    }
    const params: any = {}
    ...
    return energyKnotApi.listPaginated(page, pageSize, Object.keys(params).length > 0 ? params : undefined)
  }, [])
  ...
  const handleSave = () => {
    ...
    if (!editingKnot && formClosers.length === 0) {
      setCloserError(true)
      return
    }
    ...
    <>
            ...
            <TableBody>
              {paginatedItems.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-[#2b2f36] font-price">¥{item.amount.toLocaleString()}</TableCell>
      ...
      <Dialog open={confirmOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-sm p-0 gap-0" showCloseButton={false}>
          <DialogHeader className="px-6 pt-5 pb-4 border-b">
            <DialogTitle className="text-base">{editingKnot ? "确认编辑" : "确认新增"}</DialogTitle>
          </DialogHeader>
