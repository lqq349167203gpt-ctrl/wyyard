...
import { Plus, Trash2, Edit, Wallet, X } from "lucide-react"
import {
...
export function GroupCasesContent({ embedded }: { embedded?: boolean } = {}) {
  ...
  const fetchFn = useCallback(async (page: number, pageSize: number) => {
    if (!customersReadyRef.current) {
      return { items: [] as GroupCase[], total: 0, page: 1, page_size: pageSize, total_pages: 0 }
    }
    const params: any = {}
    ...
    return groupCaseApi.listPaginated(page, pageSize, Object.keys(params).length > 0 ? params : undefined)
  }, [])
