...
function recordResult(log: AnalysisLog) {
  ...
  return `${log.config.结果数量 ?? log.config.结果人数 ?? 0} ${log.config.结果单位 || "人"}`
}
...
function ComparisonLogDetail({ log }: { log: AnalysisLog }) {
  ...
  return <>
    ...
    <div className="grid grid-cols-[72px_1fr] gap-x-3 gap-y-2 border-t border-[#f0f0f0] pt-3">
      ...
      <span className="text-[#8f959e]">列表排列</span><span>{log.config.列表排列 || "每人显示一次"}</span>
      ...
      <Dialog open={!!selectedLog} onOpenChange={open => { if (!open) setSelectedLog(null) }}>
        <DialogContent className="max-h-[82vh] w-[620px] max-w-[92vw] gap-0 overflow-hidden p-0">
          ...
          {selectedLog && <div className="space-y-4 overflow-y-auto px-6 py-4 text-[12px]">
            ...
            {isTemplateLog(selectedLog) ? <div className="grid grid-cols-[72px_1fr] gap-x-3 gap-y-2 border-t border-[#f0f0f0] pt-3"><span className="text-[#8f959e]">模板名称</span><span className="text-[#2b2f36]">{selectedLog.config.模板名称 || "-"}</span><span className="text-[#8f959e]">模板简介</span><span className="break-words text-[#4e535a]">{selectedLog.config.模板简介 && selectedLog.config.模板简介 !== "—" ? selectedLog.config.模板简介 : "未填写"}</span><span className="text-[#8f959e]">可见范围</span><span className="text-[#2b2f36]">{selectedLog.config.可见范围 || "-"}</span><span className="text-[#8f959e]">筛选条件</span><span className="text-[#2b2f36] tabular-nums">{selectedLog.config.筛选条件数 ?? 0} 个</span><span className="text-[#8f959e]">统计指标</span><span className="text-[#4e535a]">{(selectedLog.config.统计指标 ?? []).join("、") || "-"}</span><span className="text-[#8f959e]">拆分指标</span><span className="text-[#4e535a]">{selectedLog.config.拆分指标 || "符合条件人数"}</span><span className="text-[#8f959e]">拆分维度</span><span className="text-[#4e535a]">{selectedLog.config.拆分维度 || selectedLog.config.拆分方式 || "-"}</span><span className="text-[#8f959e]">列表字段</span><span className="text-[#4e535a]">{(selectedLog.config.列表字段 ?? []).join("、") || "-"}</span><span className="text-[#8f959e]">列表排列</span><span className="text-[#4e535a]">{selectedLog.config.列表排列 || "每人显示一次"}</span></div> : isComparisonLog(selectedLog) ? <ComparisonLogDetail log={selectedLog} /> : <>
              <div className="border-t border-[#f0f0f0] pt-3"><div className="mb-2 font-medium text-[#2b2f36]">筛选范围</div><div className="grid grid-cols-[72px_1fr] gap-x-3 gap-y-2"><span className="text-[#8f959e]">时间范围</span><span>{selectedLog.config.时间范围 || "全部时间"}</span><span className="text-[#8f959e]">条件关系</span><span>{selectedLog.config.条件关系 || "全部符合"}</span><span className="text-[#8f959e]">结果数量</span><span className="tabular-nums">{selectedLog.config.结果数量 ?? selectedLog.config.结果人数 ?? 0} {selectedLog.config.结果单位 || "人"}</span></div></div>
              ...
              <div className="grid grid-cols-[72px_1fr] gap-x-3 gap-y-2 border-t border-[#f0f0f0] pt-3"><span className="text-[#8f959e]">统计指标</span><span>{(selectedLog.config.统计指标 ?? []).join("、") || "-"}</span><span className="text-[#8f959e]">拆分指标</span><span>{selectedLog.config.拆分指标 || "符合条件人数"}</span><span className="text-[#8f959e]">拆分维度</span><span>{selectedLog.config.拆分维度 || selectedLog.config.拆分方式 || "-"}</span><span className="text-[#8f959e]">显示字段</span><span>{(selectedLog.config.显示字段 ?? []).join("、") || "-"}</span><span className="text-[#8f959e]">列表排列</span><span>{selectedLog.config.列表排列 || "每人显示一次"}</span><span className="text-[#8f959e]">排序方式</span><span>{selectedLog.config.排序方式 || "-"}</span></div>
            </>}
          </div>}
        </DialogContent>
      </Dialog>
