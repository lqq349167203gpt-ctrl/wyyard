            ...
            <TableBody>
              {paginatedItems.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-[#2b2f36] font-price">¥{item.fee.toLocaleString()}</TableCell>
          ...
          <div className="px-6 py-5 space-y-4" {...enterToNext}>
            ...
            <div className="grid grid-cols-[70px_1fr] items-start gap-2">
              ...
              <div>
                ...
                {formClosers.length > 0 && formClosers.reduce((sum, c) => sum + c.amount, 0) !== parseFloat(formFee || "0") && (
      ...
      <Dialog open={confirmOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-sm p-0 gap-0" showCloseButton={false}>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-[70px_1fr] items-start gap-2">
              <span className="text-[12px] text-[#8f959e] font-light text-right tracking-widest">项目名称</span>
              <span className="text-[12px] text-[#2b2f36] pt-0.5">{formProjectName || "-"}</span>
            </div>
            <div className="grid grid-cols-[70px_1fr] items-start gap-2">
            ...
            <div className="grid grid-cols-[70px_1fr] items-start gap-2">
              <span className="text-[12px] text-[#8f959e] font-light text-right tracking-widest">活动方式</span>
              <span className="text-[12px] text-[#2b2f36] pt-0.5">{formActivityMode}</span>
            </div>
            <div className="grid grid-cols-[70px_1fr] items-start gap-2">
              <span className="text-[12px] text-[#8f959e] font-light text-right tracking-widest">费用</span>
              <span className="text-[12px] text-[#2b2f36] pt-0.5">¥{parseFloat(formFee || "0").toLocaleString()}</span>
            </div>
            <div className="grid grid-cols-[70px_1fr] items-start gap-2">
            ...
            {(formDurationValue || formRemainingCount || formUnlimited) && (
              <div className="grid grid-cols-[70px_1fr] items-start gap-2">
                <span className="text-[12px] text-[#8f959e] font-light text-right tracking-widest">有效期</span>
                <span className="text-[12px] text-[#2b2f36] pt-0.5">
                  {formDurationValue && `${formDurationValue} ${formDurationType === "month" ? "个月" : "天"}`}
                  {formDurationValue && (formRemainingCount || formUnlimited) && "，"}
                  {formUnlimited ? "次数不限" : formRemainingCount ? `${formRemainingCount} 次` : ""}
                </span>
              </div>
            )}
            <div className="grid grid-cols-[70px_1fr] items-start gap-2">
