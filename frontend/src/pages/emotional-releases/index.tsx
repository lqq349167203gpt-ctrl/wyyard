...
import { Plus, Trash2, Edit, Loader2, Heart, X } from "lucide-react"
import {
...
import { emotionalReleaseApi, type EmotionalRelease, type CustomerSearchResult } from "@/lib/api"
import { usePagination } from "@/hooks/use-pagination"
import { PaginationBar } from "@/components/pagination-bar"

export function EmotionalReleasesContent({ embedded }: { embedded?: boolean } = {}) {
  const [releases, setReleases] = useState<EmotionalRelease[]>([])
  ...
  const { paginatedItems, currentPage, totalPages, totalItems, goToPage, startIndex, endIndex } = usePagination(releases)

  const load = () => {
    emotionalReleaseApi.list()
      .then(setReleases)
      .catch(() => {})
  ...
  const handleSearch = (keyword: string, target: "user" | "closer") => {
    ...
    searchTimeoutRef.current = window.setTimeout(async () => {
      setSearching(true)
      try {
        const results = await emotionalReleaseApi.searchCustomers(keyword)
  ...
  const totalAmount = releases.reduce((sum, c) => sum + c.amount, 0)
  const totalCount = releases.reduce((sum, c) => sum + c.purchase_count, 0)
    ...
    <>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground mt-[6px]">
          {releases.length > 0 && (
            <span>共 {releases.length} 条记录，{totalCount} 次购买，¥{totalAmount.toLocaleString()}</span>
          )}
        </p>
