...
import { Plus, Trash2, Edit, Loader2, X, Calendar, Users, Heart, ChevronRight, FileUp, Download, File } from "lucide-react"
import {
...
import { emotionalReleaseSessionApi, emotionalReleaseApi, customerApi, uploadApi, type EmotionalReleaseSession, type EmotionalReleaseCustomerSearchResult, type Customer } from "@/lib/api"
...
export default function EmotionalReleaseSessionsPage() {
  ...
  const [selectedSession, setSelectedSession] = useState<EmotionalReleaseSession | null>(null)
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editingSession, setEditingSession] = useState<EmotionalReleaseSession | null>(null)
  ...
  const [panelSearchResults, setPanelSearchResults] = useState<EmotionalReleaseCustomerSearchResult[]>([])
  ...
  const handleAddParticipant = async (customer: EmotionalReleaseCustomerSearchResult) => {
    ...
    try {
      const updated = await emotionalReleaseSessionApi.update(selectedSession.id, { participant_ids: newIds })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingSession) {
        await emotionalReleaseSessionApi.update(editingSession.id, data)
      } else {
  ...
  return (
    <div className="px-6 pt-12 pb-6 space-y-3">
      {/* 页面头部 */}
      <div>
        <h1 className="text-lg font-semibold">情绪释放</h1>
      </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-sm text-muted-foreground">{filterDate ? "该日期暂无记录" : "暂无情绪释放记录"}</p>
          ...
          <div className="px-6 py-5 space-y-4 overflow-hidden">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="shrink-0">
                <input type="file" id="materials-upload-ers" className="hidden" onChange={handleUploadMaterial} />
                <Button size="sm" className="h-7 text-xs" disabled={uploading} onClick={() => document.getElementById("materials-upload-ers")?.click()}>
                  {uploading ? "上传中..." : "上传文件"}
                </Button>
              </div>
            </div>
