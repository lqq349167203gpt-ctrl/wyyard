// 归档于 2026-08-14：人员福利页面已从系统移除，历史代码保留用于数据追溯。
...
export default function StaffBenefitsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<StaffBenefitRecord | null>(null)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      await financialApi.deleteBenefit(deleteTarget.id)
  ...
  return <div className="dv-root flex h-full flex-col gap-3 bg-[#f4f5f6] p-4">
    ...
    <AlertDialog open={Boolean(deleteTarget)} onOpenChange={open => !open && setDeleteTarget(null)}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>删除人员福利记录</AlertDialogTitle>
          <AlertDialogDescription>确定删除“{deleteTarget?.benefit_date} · {deleteTarget?.content}”这条人员福利记录吗？删除后列表中将不再显示。</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
